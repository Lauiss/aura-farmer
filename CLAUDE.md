...
- **Utilitaires** : achats ponctuels qui ajoutent une mécanique plutôt qu'un bonus continu ([`UtilityManager`](src/app/services/utility-manager.ts)). Le trickshot se tire rarement au clic et fait s'envoler le combo.
...
- **Structure de la carte** : une racine « Moyai », acquise d'entrée, d'où partent deux branches — « Enseignements », qui porte les articles et leurs chaînes d'améliorations, et « Outfit », qui porte les pièces cosmétiques et les leurs ([outfit-upgrades.ts](src/assets/static/outfit-upgrades.ts)). La racine et les catégories ne s'achètent pas : `price()` renvoie 0 pour elles, car le gabarit lit leur prix pour décider des classes.
